...
export default function SignInCard() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.status === 200) {
      setOpenVerifyCode(true); // Open modal upon successful login
    }
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const result: any = await dispatch(loginAction(values.email, values.password));
      console.log('Login result:', result);
      if (result.status === 200) {
        setOpenVerifyCode(true); // Open modal upon successful login
      }
  ...
  return (
    <Card variant="outlined">
      ...
      <VerifyCodeModal open={openVerifyCode} handleClose={handleVerifyCodeClose} handleVerifyCode={handleVerifyCode} />
    </Card>
  );
}
